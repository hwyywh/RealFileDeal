    ...
    public static void processData(File sourcefile,File destfile) {//一对一    	
        ...
        int value=0;            	
           ...
           //    System.out.println("读取源文件的第"+(hang-1)+"行时间"+"DateTime "+(String) source.get("DateTime"));
               ...
               ReadFromFile.writeFile("D:\\2km范围的接触情况统计.txt",sourcefile.getName().substring(5)+","+destfile.getName().substring(5)+","+s+','+now+','+dis+'\n');//源节点+目标节点+接触时间+后一辆车的时间(接触时间的上限)
...
//               if(value!=0){
//               ReadFromFile.writeFile("D:\\countrang=2km.txt",sourcefile.getName().substring(5)+","+destfile.getName().substring(5)+","+value+'\n');
//               System.out.println("写入成功"+'\n');
//               }else{
//            	   
//               }
